...
function getConfirmedTransaction (transaction, cb) {
	http.get('/api/transactions/get?id=' + transaction.id, function (err, res) {
		...
		cb(null, res.transactions.find(function (t) {
...
describe('getSignatures', function () {
	...
	it('should return correct response', function (done) {
		ws.call('getSignature', function (err, res) {
			node.expect(res).to.have.property('success').to.be.ok;
			node.expect(res).to.have.property('transactions').to.be.an('array');
			done();
		});
	...
	before(function (done) {
		...
		var multisigMemberA = node.randomAccount();
		var multisigMemberB = node.randomAccount();

		multisigTransaction = node.lisk.multisignature.createMultisignature(multisigAccount.password, null, [multisigMemberA.publicKey, multisigMemberB.publicKey], 71, 2);
		postTransaction(multisigTransaction, done);
	});

	it('should return non signatures list after creating multisig account transaction', function (done) {
		ws.call('getTransactions', function (err, res) {
			node.expect(res).to.have.property('success').to.be.ok;
			node.expect(res).to.have.property('transactions').to.be.an('array').lengthOf(1);
			node.expect(res.transactions[0].id === multisigTransaction.id);
			done();
		});
	});
});
